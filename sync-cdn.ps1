...
# local wp-content/uploads/ files - their master copy now lives on the CDN, so
# the WordPress uploads folder is kept empty. Theme files are never deleted.
#
...
# never deletes from the CDN, so clearing the local uploads folder does not wipe
...
# robocopy returns 0-7 on success; normalise so the script does not abort.
...
if ((-not $dirty) -and ('0' -eq $unpushed)) {
    $srcUploads = Join-Path $src 'uploads'
    if (Test-Path $srcUploads) {
        Get-ChildItem -Force $srcUploads -ErrorAction SilentlyContinue |
            Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
    }
    Write-Host "Synced and pushed. Local uploads cleaned - images now served from the CDN (jsDelivr)."
